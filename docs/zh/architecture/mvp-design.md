...
title OpenPrecedent 0.1.0 MVP 系统上下文
...
@startuml
skinparam shadowing false
title OpenPrecedent 0.1.0 MVP 端到端流程
...
hide methods
hide stereotypes
title OpenPrecedent 0.1.0 MVP 核心对象
...
`0.1.0` MVP 的 precedent 检索是 case-oriented 且 lightweight 的。
...
- `0.1.0` MVP 不包含独立 graph store 或 vector store
...
## 对 0.1.0 MVP 最准确的能力总结

如果只用最短的话概括 `0.1.0` MVP，可以总结为：
