...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--openprecedent-bin",
        help="Rust OpenPrecedent CLI binary. Defaults to OPENPRECEDENT_BIN or a local target build.",
    )
    parser.add_argument(
        "--skip-if-case-exists",
        action="store_true",
...
def run_openprecedent(openprecedent_bin: str, runtime_home: Path, *args: str) -> subprocess.CompletedProcess[str]:
    env = os.environ.copy()
    env["OPENPRECEDENT_HOME"] = str(runtime_home)
    result = subprocess.run(
        [openprecedent_bin, "--home", str(runtime_home), "--format", "json", *args],
        check=False,
        capture_output=True,
        text=True,
        env=env,
...
def main() -> int:
    ...
    show_result = run_openprecedent(openprecedent_bin, runtime_home, "case", "show", case_id)
